 ...
 *Due Date: 02-06-22
 *Purpose: Caesar's Cipher decryption of messages
*******************************************************************************/
...
int main()
{
    ...
    do
    {
        ...
        if (strlen(message) > MAX_MESSAGE || strlen(message) == 0)
        {
    ...
    {
        /* Decrypts character in message if between a and z */
        if (islower(i) == 0 && message[i] >= 'a' && message[i] <= 'z')
        {
            message[i] = ((message[i] - 'a') % 26) + 'a' - key;
             /* If character is greater than a */
            if (message[i] < 96)
                message[i] += 26;
        }
        /* Decrypts character in message if between A and Z */
